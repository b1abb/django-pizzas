...
def add_to_cart(request, user_book):
    ...
    op_book = Pizza.objects.get(slug=user_book)

    try:
        check_if_already_exits = CartItems.objects.get(cart=session_aa, pizza=op_book)
        print(check_if_already_exits)
        if check_if_already_exits:
            op_book = Pizza.objects.get(slug=user_book)
            quantity_update = CartItems.objects.get(cart=session_aa, pizza=op_book)
            quantity_update = quantity_update.quantity + 1

            cartitem = CartItems.objects.get(
                cart=Cart.objects.get(cart_session=session),
                pizza=Pizza.objects.get(slug=user_book),
            )
...
def update_cart_item(request, book_slug):
    if request.POST:
        ...
        cartitem = CartItems.objects.get(
            cart=Cart.objects.get(cart_session=session),
            pizza=Pizza.objects.get(slug=book_slug),
        )
...
def delete_cart_item(request, book_slug):
    ...
    book_item = Pizza.objects.get(slug=book_slug)
    cart_items = CartItems.objects.all().filter(cart=my_cart,pizza=book_item)
    cart_items.delete()
    return redirect('cart')
...
def cart(request):
    ...
    for cart_item in cart_items:
        total += cart_item.pizza.price*cart_item.quantity
    context = {
